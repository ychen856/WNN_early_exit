...
def _parse_threshold_groups(s: str, num_exits: int) -> List[List[float]]:
    ...
    if not groups:
        return []
    return _broadcast(groups, num_exits, "cascade_thr_grid")
...
def cache_exit_features(model, loader, device, layer_idx, keep_idx, mu, sigma, use_norm: bool):
    ...
    for xb, yb in loader:
        ...
        idx = keep_idx.to(h_layer.device)
        h = h_layer[:, idx]
...
def main():
    parser = argparse.ArgumentParser(description="Train cached multi-exit heads for QuWei LUT CIFAR10 backbone.")
    ...
    for exit_id, layer_idx in enumerate(exit_layers):
        ...
        for thr in single_thr_list:
            rows_val.append(
                {"thr": thr, **eval_overall_at_thr_multi_exit(
                    model,
                    val_loader,
                    device,
                    thr=thr,
    ...
    if cascade_thr_grid:
        ...
        for thrs in itertools.product(*cascade_thr_grid):
            thrs = list(thrs)
            val_out = eval_cascade_multi_exit(
                model,
                val_loader,
                device,
                exit_heads=exit_heads,
            ...
            test_out = eval_cascade_multi_exit(
                model,
                test_loader,
                device,
                exit_heads=exit_heads,
            ...
            rows_val.append({"thrs": thrs, **val_out})
            rows_test.append({"thrs": thrs, **test_out})
