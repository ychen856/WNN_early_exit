...
class G3Config:
    num_epochs: int = 30
    lr_layer1: float = 1e-4
    lr_layer2: float = 3e-4
    lr_final: float = 3e-4
    lr_exit: float = 1e-3
    final_lambda_exit: float = 0.3
    lambda_warmup: int = 10         # ramp length (also controls schedule shape)
    weight_decay: float = 1e-3
    grad_clip: Optional[float] = 1.0
    thr_eval: float = 2.0
...
def train_g3(
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": params_layer1, "lr": cfg.lr_layer1, "weight_decay": cfg.weight_decay},
            {"params": params_layer2, "lr": cfg.lr_layer2, "weight_decay": cfg.weight_decay},
            {"params": params_final,  "lr": cfg.lr_final,  "weight_decay": cfg.weight_decay},
            {"params": params_exit,   "lr": cfg.lr_exit,   "weight_decay": cfg.weight_decay},
        ]
    )
    ...
    for ep in range(cfg.num_epochs):
        ...
        print(
            f"[G3] Ep{ep:03d} λ={lam:.3f} | "
            f"final_acc tr/va={tr_final*100:.2f}/{va_final*100:.2f} | "
            f"exit1_acc tr/va={tr_exit*100:.2f}/{va_exit*100:.2f} | "
            f"overall@thr={cfg.thr_eval} va={metrics['overall_acc']*100:.2f} "
            f"exit_rate={metrics['exit_rate']*100:.2f}"
        )
    ...
    print(f"[G3] Best val overall@thr={cfg.thr_eval}: {best_overall:.4f}")
    return model
...
if __name__ == "__main__":
    ...
    model = train_g3(model, train_loader, val_loader, device, g3_cfg)

    #print("Best val overall acc:", best["val_overall_acc"])
    save_ckpt("/Users/yi-chunchen/workspace/WNN_early_exit/model/wnn_w_exit_g3_v1.pth", model, bb_cfg, ex_cfg)
    ...
    exit_loss, exit_acc = eval_exit1_epoch(model, test_loader, device)
    print(f"[G3 Exit head only] test_acc={exit_acc*100:.2f}%")
    final_acc = eval_final_acc(model, test_loader, device)
    print(f"[G3 Final head only] test_acc={final_acc*100:.2f}%")
    m = eval_overall_at_thr(model, test_loader, device, thr=2.0)
    print(f"[G3 Overall@thr=2.0] test_acc={m['overall_acc']*100:.2f}%, exit_rate={m['exit_rate']*100:.2f}%")
    print_eval_profile("G3 Overall@thr=2.0", m)
